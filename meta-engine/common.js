...
export function getField(state, fieldPath, appDataId) {
    if (!fieldPath) {
        return state.getIn([appDataId, 'data']);
    }

    if (fieldPath instanceof Array) {
        return state.getIn([appDataId].concat(fieldPath));
    } else {
        return state.getIn([appDataId].concat(fieldPath.split('.')));
    }
}

export function getFields(state, fieldPaths, appDataId) {
    var ret = {}
    fieldPaths.forEach(o => ret[o] = getField(state, o, appDataId))
    return ret
}

export function setField(state, fieldPath, value, appDataId) {
    if (fieldPath instanceof Array) {
        return state.setIn([appDataId].concat(fieldPath), value)
    } else {
        return state.setIn([appDataId].concat(fieldPath.split('.')), value)
    }
}

export function setFields(state, values, appDataId) {
    var keys = Object.keys(values),
        i, key;

    for (i = 0; key = keys[i++];) {
        state = setField(state, key, values[key], appDataId);
    }
    return state;
}
...
function parseMeta(meta) {
    ...
    const parseProp = (propValue, parentPath, parentRealPath) => {
        ...
        if (propValue.get('name') && propValue.get('component')) {
            let name = utils.string.trim(propValue.get('name'));
