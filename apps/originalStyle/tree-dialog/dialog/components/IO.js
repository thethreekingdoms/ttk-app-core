...
export default function CardTree(props) {
  ...
  const tableData = useData([props, 'ioTableData']).toJS()
  const tableThead = useData([props, 'ioThead']).toJS()
  const [selectAll, setSelectAll] = useState(false)
  const [ioMap] = useState(['输入', '输出', '输入输出'])
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      await actions.fetchIOTableBody()
  ...
  return (
    <Layout className="ttk-table-container">
                    ...
                    <DataGrid.Cell style={{ textAlign: 'center', padding: 5 }} >
                      {
                        isEdit ?
                          <Select
                            ...
                            onChange={e => commit([props, 'ioTableData'], { type: 'update-item', data: { ...tableData[rowIndex], io: e } })} >
                            {
                              ioMap.map((mapItem, mapIndex) => <Select.Option value={mapIndex}>{mapItem}</Select.Option>)
                            }
                          </Select>
                          : <label>{ioMap[tableData[rowIndex][item.dataIndex]]}</label>
                      }
                    </DataGrid.Cell>
                  )
                case 'operate':
                  return (
                    <DataGrid.Cell style={{ textAlign: 'center', padding: 5 }} >
                      {
                        isEdit ?
                          <a onClick={() => actions.updateIOTableBody({ ...tableData[rowIndex], isEdit: false })}>保存</a>
                          : <a onClick={() => commit([props, 'ioTableData'], { type: 'update-item', data: { ...tableData[rowIndex], isEdit: true } })}>编辑</a>
                      }
                      <Divider type="vertical" />
                      <a onClick={(()=>actions.deleteIOTablItem({...tableData[rowIndex]}))}>删除</a>
                    </DataGrid.Cell>
                  )
                default:
      ...
      ></DataGrid>
      <Button icon="plus" type="dashed" onClick={()=> commit([props, 'ioTableData'], {type:'add-item', data:null})}>添加</Button>
    </Layout>
  )
}
