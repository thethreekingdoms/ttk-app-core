...
let beijingSelectOptions = [
    [ '0001129917|SXA031900185', '0001129914|SXA031900185' ],
    [ '0001129902|SXA031900512', '0001129924|SXA031900512' ],
    [ '0001011811|SXA031900831', '0001011812|SXA031900832' ],
    [ '0001013608|SXA031900394', '0001013609|SXA031900396' ]
]
let selectOptions = []
...
class action {
	...
	load = async (initData) => {
        ...
        let cloneArr = JSON.parse(JSON.stringify(defaultZzsjmssbmxbjsxm))        
        // if(zzsjmssbmxbjsxm.length < 5 ) {
        //     zzsjmssbmxbjsxm.map( item => {
        //         cloneArr[ item.ewbhxh-1 ] = item
        //     })
        //     zzsjmssbmxbjsxm = cloneArr
        // }
        ...
        this.setJsxmSelectListInfo( zzsjmssbmxbjsxm )
    ...
    setJsxmSelectListInfo = ( zzsjmssbmxbjsxm ) => {
        let haveHmc = false,
            cloneSelectList = JSON.parse( JSON.stringify( beijingSelectOptions ) )
        selectOptions = []            
        // zzsjmssbmxbjsxm.map( item => {
        //     if( item.hmc ) {
        //         haveHmc = true
        ...
        // } )
        // if( !haveHmc ) return 
        // zzsjmssbmxbjsxm.map( ( item, index ) => {
        //     if( item.hmc ) {
        //         cloneSelectList.map( ( selectItem, selectIndex ) => {
        //             if( selectItem.indexOf( item.hmc + '|' + item.swsxDm ) >= 0 ) {
        //                 selectOptions = selectOptions.concat( cloneSelectList.splice( selectIndex, 1 ) )
        //             }
        //         })
        //     }
        // } )
        selectOptions = selectOptions.concat( cloneSelectList )
    }
    ...
    initMessage = (zzsjmssbmxbjsxm, zzsjmssbmxbmsxm, checkData) => {
        ...
        if(checkData) {
            checkData.map(item => {
                ...
                if(file[1] == "zzsjmssbmxbjsxmGrid"){
                    jsMessage[file[3]] = {...jsMessage[file[3]]}
                    jsMessage[file[3]][file[4]] = item.message
                }else {
        ...
        let jsFailCount = 0, msFailCount = this.failCount(msMessage)
        this.metaAction.sfs({
    ...
    formatData = (tableName, type , zzsjmssbmxbjsxmData ) => {
        ...
        if( zzsjmssbmxbjsxmData ) {
            changeItems = [
                {
                    path: `${'zzsjmssbmxb'}_${tableName}Grid_${tableName}GridlbVO_${zzsjmssbmxbjsxmData.index}_${zzsjmssbmxbjsxmData.columnName}`,
                    oldValue: zzsjmssbmxbjsxmData.oldValue || 0,
                    newValue: zzsjmssbmxbjsxmData.newValue
                }
            ]
        } else {
    ...
    renderSelect = (tableName, index, max, value) => {
        ...
        if(index > max) {
            ...
            if(tableName == 'zzsjmssbmxbjsxm') {
                ...
                if( list && list.length ) {
                    options = selectOptions[index-1].map( item => {
                        let curItem
                        list.map( d => {
                            if( d.showData.indexOf( item ) >= 0 ) {
                                curItem = d
                            }
                        })
                        return <Option key={curItem.codeSwsxDm}>{curItem.showData}</Option>
                    })
                }
            }else {
                list = this.metaAction.gf('data.freeTax').toJS()
                options = list.map(d => <Option key={d.codeSwsxDm}>{d.showData}</Option>);
            }
    ...
    renderInput = (tableName, name, index, max, value) => {
        ...
        // let disabled = this.metaAction.gf('data.isDeclare')
        let message = this.metaAction.gf(`data.error.${tableName}`).toJS()[index]
        // if( index == 1 && tableName == 'zzsjmssbmxbjsxm' ) {
        //     let data = this.metaAction.gf(`data.zzsjmssbmxbjsxm`).toJS()
        //     return this.renderText(tableName, name, index, data[index])
        // }
        if(index > max) {
            if(message && message[name]){
                let content = message[name].map( ( p , index ) => <p className='message-p'>{index+1}、{p}</p>)
    ...
    handleBlur = (value, columnName, index, tableName) => {//焦点离开输入框触发
        ...
        //提前公共方法，处理数据
        list = this.setList( list, tableName, index, keys )
        
        this.metaAction.sf(`data.${tableName}`, fromJS(list))
        if( tableName == 'zzsjmssbmxbjsxm' ) {
            this.formatData(tableName, undefined, {
                oldValue,
                newValue: value,
                columnName,
                index,
    ...
    setList = ( list, tableName, index, keys ) => {
        if(tableName != 'zzsjmssbmxbjsxm'){//北京减税前端不计算，走后端计算
            
            list[index].kchmsxse = (parseFloat(list[index].mzzzsxmxse) || 0) - (parseFloat(list[index].bqsjkcje) || 0)
            if( mslist.indexOf( list[index].hmc.split("|")[0] ) >= 0  ) {
                list[index].mse = (parseFloat(list[index].kchmsxse) || 0) * 0.05                    
            } else {
    ...
    selectChange = (tableName, index, value) => {
        ...
        list = this.setList( list, tableName, index, [] )
