...
class action {
    ...
    componentDidMount = () => {   
        setTimeout(() => {
            let formData  = document.querySelector('.ttk-tax-app-zzsxgmGrid')
    ...
    handleBlur = (value, columnName, index, noCalc) => {//焦点离开输入框触发
        ...
        let originalTable = this.metaAction.gf( 'data.originalTable' ).toJS(),
            ...
            zzssyyxgmnsr = this.metaAction.gf('data.zzssyyxgmnsr').toJS(),
            oldValue = noCalc ? undefined : number.clearThousPos( tTable[ index ][ columnName ] ),
            tablePath = 'zzsxgmGrid_zzsxgmGridlb',
            zzsxgmGridlb,
            failCount = 0,
            path
        ...
        if( !noCalc ) {
            ...
            zzssyyxgmnsr = this.setValue( path, value, zzssyyxgmnsr )
            zzsxgmGridlb = zzssyyxgmnsr.zzsxgmGrid.zzsxgmGridlb
            tTable = this.getTableData( tTable, zzsxgmGridlb, mapRelation )

            path = 'zzssyyxgmnsr_' + path.replace('[','_').replace(']','')
            this.metaAction.sf( 'data.tTable', fromJS( tTable ) )        
        } else {
            oldValue = zzssyyxgmnsr.slxxForm[ columnName ]
            zzssyyxgmnsr.slxxForm[ columnName ] = value
            path = 'zzssyyxgmnsr_slxxForm_'+columnName
        }

        let setVOData = this.metaAction.gf( 'data.setVOData' ) 
        if(zzssyyxgmnsr.slxxForm.sfzxsb=="N"&&!zzssyyxgmnsr.slxxForm.blrysfzjlxDm) {
            failCount +=1
        }
        if(zzssyyxgmnsr.slxxForm.sfzxsb=="N"&&!zzssyyxgmnsr.slxxForm.blrysfzjhm) {
            failCount +=1
        }
        if(zzssyyxgmnsr.slxxForm.sfzxsb==="N"&&!zzssyyxgmnsr.slxxForm.slrxm) {
            failCount +=1
        }
        let res = setVOData( {
            changeTable: zzssyyxgmnsr,
            changeItems : [
                {
                    path: path,
    ...
    getPath = (path, columnName, index, mapRelation) => {
        let colNum = '['+(parseInt( columnName.substr(1) )-1)+']',
            zzssyyxgmnsr = mapRelation.zzssyyxgmnsr,
            rowName
        for(let key in zzssyyxgmnsr ) {
            if( zzssyyxgmnsr[key] == index+1 ) {
                rowName = key
            }
        }
    ...
    setValue = ( path , value , zzssyyxgmnsr ) => {
        let pathArr = path.split('_').join('.'),
            str = 'zzssyyxgmnsr.'+pathArr+'='+value
        eval(str)
        return zzssyyxgmnsr
    }
        ...
        zzsxgmGridlb.map( (item, index) => {
            ...
            for(let key in item ) {
                if( key !='ewblxh' && key != 'lmc') {
                    ...
                    tTable[ colMap[key]-1 ]['C'+indexStr ] = item[key]
                }
            }
        })
        return tTable
