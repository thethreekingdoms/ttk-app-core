...
import { Form, Popover, LoadingMask, Input, FormDecorator } from 'edf-component'
...
class action {
    ...
    renderFormCell = (columnName, value, type ) => {
        ...
        if( checkData ) {
            checkData.map( item => {
                if( item.target == 'zzsxgmflzl_flzlForm_'+columnName ) {
    ...
    handleBlur = (value, columnName) => {
        ...
        let changeTable = { flzlForm: this.metaAction.gf('data.flzlForm').toJS() }
        let changeItems= [{
            path: `zzsxgmflzl_flzlForm_${columnName}`,
        ...
        changeTable.flzlForm[columnName] = value
        let res = this.component.props.setVOData({changeTable, changeItems})
        let newData = {}
        newData[ `data.flzlForm` ] = fromJS( res.changeData.flzlForm )
