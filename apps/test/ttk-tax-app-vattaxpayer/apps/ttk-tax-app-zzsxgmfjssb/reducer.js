...
class reducer {
    ...
    init = (state, option, setVOData ) => {
        ...
        let mapRelation = option.mapRelation, //映射关系
            ...
            zzsxgmfjssb = option.zzsxgmfjssb || {},//主表表格数据
            xgmfjsxxGrid = zzsxgmfjssb.xgmfjsxxGrid,
            xgmfjsxxGridlb = xgmfjsxxGrid.xgmfjsxxGridlb,
        ...
        initState.data.tTable = this.getTableData( initState.data.tTable , xgmfjsxxGridlb, mapRelation )
        initState.data.zzsxgmfjssb = option.zzsxgmfjssb ? option.zzsxgmfjssb : {
            sbbhead:{},
            xgmfjsxxGrid:{}
        }
    ...
    load = (state, initData, setVOData ) => {
        let data = this.metaReducer.gf(state, 'data').toJS(),
            ...
            zzsxgmfjssb = initData.zzsxgmfjssb || {},//主表表格数据
            xgmfjsxxGrid = zzsxgmfjssb.xgmfjsxxGrid,
            xgmfjsxxGridlb = xgmfjsxxGrid.xgmfjsxxGridlb,
            checkData = initData.checkData,
            originalMeta,
            originalMeta2 = fromJS( data.originalMeta ),
        ...
        data.tTable = this.getTableData( data.tTable , xgmfjsxxGridlb, mapRelation )
        data.zzsxgmfjssb = initData.zzsxgmfjssb ? initData.zzsxgmfjssb : {
            sbbhead:{},
        }
        data.mapRelation = mapRelation
        data.sbbhead = sbbhead
        if(setVOData) {
    ...
    setData = (state, resData ) => {
        let data = this.metaReducer.gf(state, 'data').toJS(),
            ...
            // zzsxgmfjssb = changeData.zzsxgmfjssb || {},//主表表格数据
            xgmfjsxxGrid = changeData.xgmfjsxxGrid,
            xgmfjsxxGridlb = xgmfjsxxGrid.xgmfjsxxGridlb,
            tTable = data.tTable
        ...
        state = this.metaReducer.sf(state, 'data.zzsxgmfjssb', fromJS(changeData) )
        return state
    }
    ...
    changeRenderMerge( checkData , originalMeta , mapRelation ) {
        ...
        checkData.map( item => {
            let path = item.target,
                ...
                zzsxgmfjssbMap = mapRelation.zzsxgmfjssb,
                pathArr = path.split('_'),
                colName = 'C0'+( parseInt( pathArr[ pathArr.length-2 ] )+ 1),
                rowNum = zzsxgmfjssbMap[ pathArr[ pathArr.length-1 ] ] - 1,
                haveRItem = false
