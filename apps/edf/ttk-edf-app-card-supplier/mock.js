...
fetch.mock('/v1/ba/supplier/create', (option) => {
	const id = option.code
	const v = {...option, id}
	mockData.supplier.list.push(v)
	return {result: true, value: v}
})

fetch.mock('/v1/ba/supplier/query', (option) => {
	let supplier = mockData.supplier.list, v
...
fetch.mock('/v1/ba/supplier/update', (option) => {
	let supplier = mockData.supplier.list, v
	supplier.forEach((data, index) => {
		if (option.id == data.id) {
			v = index
		}
	})
