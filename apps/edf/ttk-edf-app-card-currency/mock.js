...
fetch.mock('/v1/ba/currency/create', (option) => {
	const id = option.code
	const v = {...option, id}
	mockData.currency.list.push(v)
	return {result: true, value: v}
})

fetch.mock('/v1/ba/currency/query', (option) => {
	let currency = mockData.currency.list, v
...
fetch.mock('/v1/ba/currency/update', (option) => {
	let currency = mockData.currency.list, v
	currency.forEach((data, index) => {
		if (option.id == data.id) {
			v = index
		}
	})
