...
class reducer {
    ...
    init = (state, option) => {
        ...
        if(availableOrg == 1) {
            ...
            state = this.metaReducer.sf(state, 'data.isShowMenu', false)
            state = this.metaReducer.sf(state, 'data.visible', false)
            return this.setContent(state, '新建企业', 'ttk-edf-app-company-manage')
        }
        return state
    }
    ...
    load = (state, { menu, manageList }) => {
        ...
        const childApp = history.getChildApp('ttk-edf-app-portal-hor')
    ...
    setContent = (state, name, appName, appProps) => {
        ...
        if (orgSign) {
            setTimeout(() => {
                history.pushChildApp('ttk-edf-app-portal-hor')
            }, 0)
        }
        else {
            setTimeout(() => {
                history.pushChildApp('ttk-edf-app-portal-hor', content.get('appName'))
            }, 0)
        }
