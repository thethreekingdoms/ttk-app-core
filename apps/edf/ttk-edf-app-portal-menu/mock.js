...
function query(option) {
    ...
    let totalPage = Math.round(totalData / pageSize) + (totalData % pageSize ? 1 : 0)
    ...
    let ret = {
        result: true,
        value: {
            page: { currentPage, pageSize, totalPage ,totalData},
            list: []
        }
    }
...
fetch.mock('/v1/edf/menu/deleteBatch', (option) => {
    mockData.menu = mockData.menu.filter((m) => {
        option.find(id => id == m.id) === undefined
    })
